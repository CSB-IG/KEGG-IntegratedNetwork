...
plot_all_network <- function(nodes.file, pdf.file){
  pdf(pdf.file, onefile = T)
  
  nodes.all <- read.table(nodes.file, sep=",", header=T, stringsAsFactors = F)
  nodes.no.isolated <- subset(nodes.all, !is.na(Degree) & Degree > 0)
  
  ######### LOG-LOG DEGREE DISTRIBUTION  
  
  table.k.log <- table(log10(nodes.no.isolated$Degree))
...
plot_gcc_network <- function(nodes.file, edges.file, pdf.file){
  pdf(pdf.file, onefile = T)
