...
def get_red_vias(interactions, path, rep, out_dir):
    out_file =  out_dir + "/nodes_rep_" + str(rep) + ".txt"
    out = open(out_file, 'w')
    vias_connected = {}
...
###### ORDENAN LOS NODOS DE MAYOR A MENOR DEGREE        
nodes_degree_sorted = sorted(nodes_degree, reverse=True)
nodes_names_sorted = [x for _,x in sorted(zip(nodes_degree,nodes_names), reverse=True)]

#print (nodes_degree_sorted)
#print (nodes_names_sorted)

#### SE ELIMINAN LOS PRIMEROS K NODOS
new_nodes = nodes_names_sorted[k:len(nodes_names_sorted)]
